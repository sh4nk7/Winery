...
import java.util.*;
    ...
    public class Winehouse {
        private static final String DBURL = "jdbc:mysql://127.0.0.1:3306/wines?";
        private static final String ARGS = "createDatabaseIfNotExist=true&serverTimezone=UTC";
        private static final String LOGIN = "winery_user";      //winery_user
        private static final String PASSWORD = "Winery2020!";  //era Winery2020!
        ...
        public ObservableList<User> getMembers(){
            ...
            try (Connection conn = DriverManager.getConnection(DBURL + ARGS, LOGIN, PASSWORD);Statement stmt = conn.createStatement()){
                ...
                ResultSet rset = stmt.executeQuery(strSelect);
                while (rset.next()){
                    allMembers.add(new User(rset.getString("USERNAME"), rset.getString("PASSWORD"), rset.getInt("NEWLY_ARRIVED")));
                }
            } catch (SQLException sqle) {
                sqle.printStackTrace();
            }
        ...
        public boolean removeWine(Seller authorizer, Wine toRemove, Integer quantity) {
            ...
            ObservableList<Wine> wines = getWineByNameAndYear(toRemove.getName(), toRemove.getYear());
            if(wines.size() > 0) {
                Wine searched = wines.get(0);
