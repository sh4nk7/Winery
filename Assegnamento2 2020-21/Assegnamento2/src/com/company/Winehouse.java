...
public class Winehouse {
    ...
    public Person login(String username, String password) {
        // First, we check if the person is a normal user
        ...
        // If is not a normal user, let's check if it's a seller
     ...
     * @return the results
     */
    public HashMap<Wine, InventoryItem> findWinesByName(Person authorizer, String toSearch) {
     ...
     * @return the results
     */
    public HashMap<Wine, InventoryItem> findWinesByYear(User authorizer, Integer toSearch) {
        if (!this.members.contains(authorizer)) {
            return null;
        }
        HashMap<Wine, InventoryItem> searched = new HashMap<Wine, InventoryItem>();
        for (Map.Entry<Wine, InventoryItem> temp : this.wines.entrySet()) {
            if (temp.getValue().getQuantityByYear(toSearch) > 0) {
                searched.put(temp.getKey(), temp.getValue());
            }
        }
        return null;
    ...
    public void addWine(Seller authorizer, Wine toAdd, Integer year, Integer quantity) {
        ...
        if (this.wines.containsKey(toAdd)) {
            this.wines.get(toAdd).modifyQuantity(year, quantity);
        } else {
    ...
    public boolean removeWine(Seller authorizer, Wine toRemove, Integer year) {
        ...
        if (this.wines.containsKey(toRemove) && this.sellers.contains(authorizer)) {
            return (this.wines.get(toRemove).modifyQuantity(year, -1));
        }
        return false;
    }
    ...
    public boolean removeWine(Seller authorizer, Wine toRemove, Integer year, Integer quantity) {
        //check authorizer
        if (this.wines.containsKey(toRemove) && this.sellers.contains(authorizer)) {
            return (this.wines.get(toRemove).modifyQuantity(year, quantity * -1));
        }
        return false;
    }
     ...
     * @param authorizer
     */
    public void shipWinesOrders(Seller authorizer) {
     ...
     * @return the orders
     */
    public ArrayList<Order> getOrdersByUser(User requester) {
